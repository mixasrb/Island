...
void gekko_t::decodeExecute(const uint32_t& opcode) {
	...
	case 0b010000:
		...
		break;
	case 0b010010:
		bx((opcode ^ 0x48000000) >> 2, (opcode >> 1) & 0x1, opcode & 0x1);
		break;
	case 0b010011:
	...
	case 0b011111:
		...
		case 0b0001010110:
			...
			dcbf((opcode >> 16) & 0x1f, (opcode >> 11) & 0x1f);
			break;
		case 0b1001101000:
		case 0b0001101000:
			negx((opcode >> 21) & 0x1f, (opcode >> 16) & 0x1f, (opcode >> 10) & 0x1, opcode & 0x1);
			break;
		case 0b0001111100:
		...
		case 0b0111010110:
			...
			dcbi((opcode >> 16) & 0x1f, (opcode >> 11) & 0x1f);
			break;
		case 0b1001010110:
		...
		case 0b1111010110:
			...
			icbi((opcode >> 16) & 0x1f, (opcode >> 11) & 0x1f);
			break;
		default:
			//g_pause = true;
			std::cout << "Fatal Error: Unhandled decode of opcode 0x" << std::hex << opcode << "\n";
...
#define setCR(a, b, crf)\
	...
	if (a < b)\
		...
		c = 0b001;\
	\
	const uint32_t x = ~0u >> (4 * crf);\
	const uint32_t y = ~0u << (31 - (4 * crf + 3));\
...
void gekko_t::subfic(const uint8_t& D, const uint8_t& A, const int16_t& SIMM) { //
	...
	uint64_t result = uint64_t(~GPR[A]) + uint64_t(int32_t(SIMM)) + 1lu;

	GPR[D] = uint32_t(result);
...
void gekko_t::cmpli(const uint8_t& crfD, const uint8_t& L, const uint8_t& A, const uint16_t& UIMM) {
	...
	const uint32_t b = uint32_t(UIMM);
	setCR(a, b, crfD);
}
...
void gekko_t::addicp(const uint8_t& D, const uint8_t& A, const int16_t& SIMM) {
	...
	/*uint64_t result = uint64_t(GPR[A]) + uint64_t(int32_t(SIMM));
...
void gekko_t::bcx(const uint8_t& BO, const uint8_t& BI, const int16_t& BD, const uint8_t& AA, const uint8_t& LK) { //
	...
	bool ctr_ok = (BO & 0x04) || ((CTR != 0) != ((BO >> 1) & 0x01));
	bool cond_ok = (BO & 0x10) || (((CR >> (31 - BI)) & 0x01) == ((BO >> 3) & 0x01));
...
void gekko_t::dcbf(const uint8_t& A, const uint8_t& B) {
	uint32_t b = 0;
	if (A)
		b = GPR[A];
...
void gekko_t::dcbi(const uint8_t& A, const uint8_t& B) {
	uint32_t b = 0;
	if (A)
		b = GPR[A];
...
void gekko_t::icbi(const uint8_t& A, const uint8_t& B) {
	uint32_t b = 0;
	if (A)
		b = GPR[A];
